...
async def check_edit_media(event):
    ...
    if event.is_group: #Write Code By T.me/zzzzl1l
        ...
        if not admin and not creator:
            return
    zed_dev = (7205408861, 7205408861, 7205408861, 7205408861, 7205408861)  #Write Code By T.me/zzzzl1l
    zelzal = event.sender_id
    malath = zedub.uid
    hhh = event.message.text
    zed_id = event.chat_id
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - ZelZal" "زلـزال الهيبـه"
    zed_dev = (7205408861, 7205408861, 7205408861, 7205408861, 7205408861)
    malath = zedub.uid
...
async def _(event):
    ...
    zed_dev = (7205408861, 7205408861, 7205408861)
        ...
        if event.user_joined:
            ...
            if zedy.id in zed_dev:
	            ...
	            try:
	                ...
	                await event.reply(f"[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𓏺𝙖𝙡𝙜𝙩𝙝𝙤𝙣. - حمـاية المجموعـة ](t.me/https://atared.serv00.net/s/rq.php?text=)\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n⌔╎**عـذࢪًا** [{zedy.first_name}](tg://user?id={zedy.id})  \n⌔╎**يُمنـع الانضمـام لـ هـذه المجموعـة 🚷•**\n⌔╎**تـم حظـࢪه .. بنجـاح ☑️**", link_preview=False)
	            except Exception as e:
	                await event.reply(
	                    "**⎆ عـذࢪًا  عـزيـزي .. لا املك صـلاحيات المشـرف هنـا 𓆰** \n`{}`".format(
	                        str(e)
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𓏺𝙖𝙡𝙜𝙩𝙝𝙤𝙣.\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𓏺𝙖𝙡𝙜𝙩𝙝𝙤𝙣.\
